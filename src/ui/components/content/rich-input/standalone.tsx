...
export const RichInputBase = forwardRef<RichInputBaseHandle, RichInputBaseProps>(function RichInputBase({
  ...
  const editor = useEditor({
    ...
    // Tiptap throws outright when it detects SSR without this, so any consumer
    // that renders a composer inside an SSR'd page (a reply box under a thread,
    // say) would crash the whole route. Deferring the first render to the
    // client is also what avoids a hydration mismatch, since the editor's DOM
    // is generated by ProseMirror rather than by React.
