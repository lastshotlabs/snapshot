...
function TierCard({
  ...
  const cardS = resolveSurfacePresentation({
    surfaceId: cardId,
    implementationBase: {
      display: "flex",
  ...
  const badgeS = resolveSurfacePresentation({
  ...
  const nameS = resolveSurfacePresentation({
  ...
  const priceRowS = resolveSurfacePresentation({
    surfaceId: `${cardId}-priceRow`,
    implementationBase: { display: "flex", alignItems: "baseline", gap: "xs" },
    componentSurface: slots?.priceRow,
  });
  const priceS = resolveSurfacePresentation({
  ...
  const periodS = resolveSurfacePresentation({
  ...
  const descS = resolveSurfacePresentation({
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className={priceRowS.className}
          style={priceRowS.style}
        >
          <span
            data-snapshot-id={`${cardId}-price`}
            className={priceS.className}
            style={priceS.style}
          >
            {priceDisplay}
          </span>
        ...
        >
          {tier.features.map((feature, fi) => {
            ...
            const iconS = resolveSurfacePresentation({
              surfaceId: `${cardId}-feature-${fi}-icon`,
              implementationBase: {
                color: included
            ...
            const textS = resolveSurfacePresentation({
              surfaceId: `${cardId}-feature-${fi}-text`,
              implementationBase: {
                style: {
            ...
            return (
              <li
                key={fi}
                data-snapshot-id={`${cardId}-feature-${fi}`}
                className={itemS.className}
                style={itemS.style}
              >
                <span
                  aria-hidden="true"
                  data-snapshot-id={`${cardId}-feature-${fi}-icon`}
                ...
                <span
                  data-snapshot-id={`${cardId}-feature-${fi}-text`}
                ...
                <SurfaceStyles css={iconS.scopedCss} />
                <SurfaceStyles css={textS.scopedCss} />
              </li>
            );
          })}
        </ul>
        <ButtonControl
          ...
          disabled={!tier.onAction}
          surfaceId={`${cardId}-cta`}
          variant="ghost"
          size="sm"
          surfaceConfig={ctaS.resolvedConfigForWrapper}
        >
          {ctaLabel}
        </ButtonControl>
      </div>
      <SurfaceStyles css={cardS.scopedCss} />
      <SurfaceStyles css={badgeS.scopedCss} />
      <SurfaceStyles css={nameS.scopedCss} />
      <SurfaceStyles css={priceRowS.scopedCss} />
      <SurfaceStyles css={priceS.scopedCss} />
      <SurfaceStyles css={periodS.scopedCss} />
      <SurfaceStyles css={descS.scopedCss} />
...
export function PricingTableBase({
  ...
  if (variant === "table") {
    ...
    const tableRootS = resolveSurfacePresentation({
      surfaceId: rootId,
      implementationBase: {
        overflow: "hidden",
        borderRadius: "lg",
    ...
    const scrollS = resolveSurfacePresentation({
      surfaceId: `${rootId}-tableScroll`,
      implementationBase: { overflow: "auto" },
      componentSurface: slots?.tableScroll,
    });
    const tableS = resolveSurfacePresentation({
    ...
    return (
      <>
        <div
          ...
          className={tableRootS.className}
          style={tableRootS.style}
        >
          <div
            data-testid="pricing-table-grid"
            data-snapshot-id={`${rootId}-tableScroll`}
            className={scrollS.className}
            style={scrollS.style}
          >
            <table
            ...
            >
              ...
              <tbody>
                {allFeatures.map((ft, fi) => (
                  <tr key={fi}>
                    <td
                      style={{
                        padding: "var(--sn-spacing-xs) var(--sn-spacing-md)",
                        borderBottom:
                          "var(--sn-border-default, 1px) solid var(--sn-color-border, #e5e7eb)",
                        fontSize: "var(--sn-font-size-sm)",
                      }}
                    ...
                    {tiers.map((t, ti) => {
                      const f = t.features.find((e) => e.text === ft);
                      const inc = f ? f.included !== false : false;
                      return (
                        <td
                          key={ti}
                          style={{
                            textAlign: "center",
                            padding:
                              "var(--sn-spacing-xs) var(--sn-spacing-md)",
                            borderBottom:
                              "var(--sn-border-default, 1px) solid var(--sn-color-border, #e5e7eb)",
                            color: inc
                              ? "var(--sn-color-success, #16a34a)"
                  ...
                  {tiers.map((t, ti) => (
                    ...
                    >
                      <ButtonControl
                        ...
                        onClick={t.onAction}
                        disabled={!t.onAction}
                        surfaceId={`${rootId}-cta-${ti}`}
                        variant="ghost"
                        size="sm"
                      >
                        {t.actionLabel ?? "Get Started"}
                      </ButtonControl>
                    </td>
                  ))}
                </tr>
        ...
        <SurfaceStyles css={tableRootS.scopedCss} />
        <SurfaceStyles css={scrollS.scopedCss} />
        <SurfaceStyles css={tableS.scopedCss} />
      </>
    );
  }
