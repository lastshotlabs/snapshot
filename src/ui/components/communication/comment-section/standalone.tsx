...
function CommentSkeleton({
  ...
  const itemS = resolveSurfacePresentation({
    surfaceId: itemId,
    implementationBase: { display: "flex", gap: "sm", paddingY: "sm" },
    componentSurface: slots?.loadingItem,
  });
  const avS = resolveSurfacePresentation({
  ...
  const tiS = resolveSurfacePresentation({
  ...
  const boS = resolveSurfacePresentation({
    ...
    <>
      ...
      >
        <div
          ...
          className={avS.className}
          style={avS.style}
        />
        <div style={{ flex: 1 }}>
          <div
            data-snapshot-id={`${itemId}-title`}
            className={tiS.className}
            style={tiS.style}
          />
          <div
            data-snapshot-id={`${itemId}-body`}
            className={boS.className}
            style={boS.style}
          />
        </div>
      </div>
      <SurfaceStyles css={itemS.scopedCss} />
      <SurfaceStyles css={avS.scopedCss} />
      <SurfaceStyles css={tiS.scopedCss} />
      <SurfaceStyles css={boS.scopedCss} />
    </>
  );
}
...
function CommentItem({
  ...
  const itemS = resolveSurfacePresentation({
    surfaceId: itemId,
    implementationBase: { display: "flex", gap: "sm", paddingY: "sm" },
    componentSurface: slots?.commentItem,
  });
  const avImgS = resolveSurfacePresentation({
  ...
  const avFbS = resolveSurfacePresentation({
  ...
  const hdS = resolveSurfacePresentation({
    ...
    implementationBase: {
      ...
      alignItems: "baseline",
      gap: "sm",
      style: { marginBottom: "var(--sn-spacing-2xs, 0.125rem)" },
    },
    componentSurface: slots?.commentHeader,
  });
  ...
  const delS = resolveSurfacePresentation({
    ...
    componentSurface: slots?.deleteButton,
  });
  ...
  return (
    <>
      ...
      >
        {authorAvatar ? (
          <img
            ...
            className={avImgS.className}
            style={avImgS.style}
          />
        ) : (
          <div
            data-snapshot-id={`${itemId}-avatarFallback`}
            className={avFbS.className}
            style={avFbS.style}
          >
        ...
        >
          ...
          >
            ...
            {ts ? (
              ...
              >
                {formatRelativeTime(ts, { short: true })}
              </span>
            ) : null}
            {showDelete && onDelete ? (
              <ButtonControl
                ...
                surfaceId={`${itemId}-deleteButton`}
                surfaceConfig={delS.resolvedConfigForWrapper}
      ...
      <SurfaceStyles css={itemS.scopedCss} />
      <SurfaceStyles css={avImgS.scopedCss} />
      <SurfaceStyles css={avFbS.scopedCss} />
      ...
      <SurfaceStyles css={delS.scopedCss} />
...
export function CommentSectionBase({
  ...
  const rootS = resolveSurfacePresentation({
    surfaceId: rootId,
    implementationBase: {
      overflow: "hidden",
      borderRadius: "md",
  ...
  const headerS = resolveSurfacePresentation({
  ...
  const hIconS = resolveSurfacePresentation({
    surfaceId: `${rootId}-headerIcon`,
    implementationBase: { color: "var(--sn-color-muted-foreground, #6b7280)" },
    componentSurface: slots?.headerIcon,
  });
  const hTitleS = resolveSurfacePresentation({
  ...
  const hCountS = resolveSurfacePresentation({
    surfaceId: `${rootId}-headerCount`,
    implementationBase: {
      fontSize: "xs",
      color: "var(--sn-color-muted-foreground, #6b7280)",
    },
    componentSurface: slots?.headerCount,
  });
  const listS = resolveSurfacePresentation({
  ...
  const errS = resolveSurfacePresentation({
  ...
  const emptyS = resolveSurfacePresentation({
  ...
  const inputS = resolveSurfacePresentation({
    ...
    componentSurface: slots?.inputArea,
  });
  ...
  return (
    <>
      <div
        ...
        className={rootS.className}
        style={rootS.style}
      >
        <div
          data-snapshot-id={`${rootId}-header`}
          className={headerS.className}
          style={headerS.style}
        >
          <span
            aria-hidden="true"
            data-snapshot-id={`${rootId}-headerIcon`}
            className={hIconS.className}
            style={hIconS.style}
          >
        ...
        >
          {loading ? (
            <>
              <CommentSkeleton rootId={rootId} index={0} slots={slots} />
              <CommentSkeleton rootId={rootId} index={1} slots={slots} />
            </>
          ) : null}
          {error ? (
            <div
              data-snapshot-id={`${rootId}-errorState`}
              className={errS.className}
              style={errS.style}
            >
              {error}
            </div>
          ...
          {!loading && !error
            ? sorted.map((comment, idx) => {
                const wS = resolveSurfacePresentation({
                  surfaceId: `${rootId}-commentWrapper-${idx}`,
                  implementationBase: {
                    style: {
                      borderBottom:
                        idx < sorted.length - 1
                  ...
                  componentSurface: slots?.commentWrapper,
                });
                return (
                  <div
                    key={
                    ...
                    data-snapshot-id={`${rootId}-commentWrapper-${idx}`}
                  ...
                  >
                    <CommentItem
                      ...
                      authorAvatarField={authorAvatarField}
                      ...
                      slots={slots}
                    />
                    <SurfaceStyles css={wS.scopedCss} />
                  </div>
                );
              })
            : null}
      ...
      <SurfaceStyles css={headerS.scopedCss} />
      <SurfaceStyles css={hIconS.scopedCss} />
      <SurfaceStyles css={hTitleS.scopedCss} />
      <SurfaceStyles css={hCountS.scopedCss} />
      <SurfaceStyles css={listS.scopedCss} />
      <SurfaceStyles css={errS.scopedCss} />
      <SurfaceStyles css={emptyS.scopedCss} />
      <SurfaceStyles css={inputS.scopedCss} />
    </>
  );
}
