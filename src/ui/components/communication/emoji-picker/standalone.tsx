...
export function EmojiPickerBase({
  ...
  const categoryTabsSurface = resolveSurfacePresentation({
    ...
    componentSurface: slots?.categoryTabs,
  });
  const gridScrollSurface = resolveSurfacePresentation({
    ...
    componentSurface: slots?.gridScroll,
  });
          ...
          {categoryKeys.map((key) => {
            ...
            return (
              <div key={key}>
                <ButtonControl
                  role="tab"
                  ariaSelected={activeCategory === key}
                  ariaControls={`${rootId}-tabpanel`}
                  type="button"
                  title={CATEGORY_LABELS[key] ?? key}
                  ariaLabel={CATEGORY_LABELS[key] ?? key}
                  onClick={() =>
          ...
          {filteredCategories
            ...
            .map((category) => {
              ...
              const sectionSurface = resolveSurfacePresentation({
                surfaceId: catId,
                implementationBase: {
                  style: { marginBottom: "var(--sn-spacing-xs, 0.25rem)" },
                },
                componentSurface: slots?.categorySection,
              });
              const labelSurface = resolveSurfacePresentation({
                surfaceId: `${catId}-label`,
                implementationBase: {
                  paddingY: "xs",
                  paddingX: "xs",
                  fontSize: "xs",
              ...
              return (
                ...
                >
                  ...
                  <div
                    role="grid"
                    data-snapshot-id={`${catId}-grid`}
                    className={gridSurface.className}
                    style={gridSurface.style}
                  >
                    {category.emojis.map((emoji) => {
                      const custom = (
                        emoji as unknown as Record<string, unknown>
                      )._custom as CustomEmoji | undefined;
                      const btnSurface = resolveSurfacePresentation({
                        surfaceId: `${catId}-emoji-${emoji.name}`,
                        implementationBase: {
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                        ...
                        componentSurface: slots?.emojiButton,
                      });
                      const customSurface = resolveSurfacePresentation({
                        surfaceId: `${catId}-emoji-${emoji.name}-custom`,
                        implementationBase: {
                          style: {
                            width: "1.375rem",
                            height: "1.375rem",
                        ...
                        componentSurface: slots?.customEmoji,
                      });
                      return (
                        <div key={emoji.name}>
                          <ButtonControl
                            ...
                            title={
                              custom ? `:${custom.shortcode}:` : emoji.name
                            }
                            ariaLabel={
                              custom ? `:${custom.shortcode}:` : emoji.name
                            }
                            surfaceId={`${catId}-emoji-${emoji.name}`}
                            surfaceConfig={btnSurface.resolvedConfigForWrapper}
                            variant="ghost"
                            size="icon"
                          >
                            {custom ? (
                              <img
                                ...
                                data-snapshot-id={`${catId}-emoji-${emoji.name}-custom`}
                                className={customSurface.className}
                                style={customSurface.style}
                              />
                            ) : (
                              emoji.native
                            )}
