...
function MessageAvatar({
  ...
  if (src) {
    const s = resolveSurfacePresentation({
...
function MessageSkeleton({
  ...
  const row = resolveSurfacePresentation({
    surfaceId: rowId,
    implementationBase: {
      display: "flex",
      gap: "sm",
  ...
  const av = resolveSurfacePresentation({
  ...
  const ti = resolveSurfacePresentation({
  ...
  const bo = resolveSurfacePresentation({
  ...
  return (
    <>
      <div data-snapshot-id={rowId} className={row.className} style={row.style}>
        <div
      ...
      <SurfaceStyles css={row.scopedCss} />
      <SurfaceStyles css={av.scopedCss} />
      <SurfaceStyles css={ti.scopedCss} />
      <SurfaceStyles css={bo.scopedCss} />
    </>
  );
}
    ...
    <>
      ...
      >
        ...
        >
          {loading ? (
            <>
              <MessageSkeleton rootId={rootId} index={0} slots={slots} />
              <MessageSkeleton rootId={rootId} index={1} slots={slots} />
              <MessageSkeleton rootId={rootId} index={2} slots={slots} />
            </>
          ) : null}
          {error ? (
            <div
                ...
                <div key={group.dateLabel || gi}>
                  {group.dateLabel
                    ? (() => {
                        ...
                        const sepS = resolveSurfacePresentation({
                          surfaceId: sepId,
                          implementationBase: {
                            display: "flex",
                            alignItems: "center",
                          ...
                          componentSurface: slots?.dateSeparator,
                        });
                        const ruleS = resolveSurfacePresentation({
                          surfaceId: `${sepId}-rule`,
                          implementationBase: {
                            bg: "var(--sn-color-border, #e5e7eb)",
                            flex: "1",
                            style: { height: "1px" },
                          },
                          componentSurface: slots?.dateRule,
                        });
                        const labS = resolveSurfacePresentation({
                          surfaceId: `${sepId}-label`,
                          implementationBase: {
                            fontSize: "xs",
                            fontWeight: "medium",
                            color: "var(--sn-color-muted-foreground, #6b7280)",
                          ...
                          componentSurface: slots?.dateLabel,
                        });
                        return (
                          <div
                            data-testid="date-separator"
                            data-snapshot-id={sepId}
                            className={sepS.className}
                            style={sepS.style}
                          >
                            <div
                              data-snapshot-id={`${sepId}-rule`}
                              className={ruleS.className}
                              style={ruleS.style}
                            />
                            <span
                              data-snapshot-id={`${sepId}-label`}
                              className={labS.className}
                              style={labS.style}
                            >
                              {group.dateLabel}
                            </span>
                            <div
                              data-snapshot-id={`${sepId}-rule-tail`}
                              className={ruleS.className}
                              style={ruleS.style}
                            />
                            <SurfaceStyles css={sepS.scopedCss} />
                            <SurfaceStyles css={ruleS.scopedCss} />
                            <SurfaceStyles css={labS.scopedCss} />
                          </div>
                        );
                      })()
                    : null}
                  {group.messages.map((msg, mi) => {
                    ...
                    const prevAuthor = prev
                      ? String(getNestedField(prev, authorNameField) ?? "")
                      : "";
                    const prevTs = prev
                      ? getNestedField(prev, timestampField)
                      : null;
                    ...
                    const isGrouped =
                      ...
                      prevTime != null &&
                      ts != null &&
                      ts.getTime() - prevTime.getTime() < 5 * 60 * 1000;
                    const msgS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}`,
                      implementationBase: {
                        display: "flex",
                        gap: "sm",
                        ...
                        style: {
                          padding: isGrouped
                            ? "var(--sn-spacing-2xs, 0.125rem) var(--sn-spacing-md, 1rem) var(--sn-spacing-2xs, 0.125rem) calc(32px + var(--sn-spacing-sm, 0.5rem) + var(--sn-spacing-md, 1rem))"
                            : "var(--sn-spacing-sm, 0.5rem) var(--sn-spacing-md, 1rem)",
                          transition:
                            "background-color var(--sn-duration-fast, 150ms) var(--sn-ease-default, ease)",
                        },
                      },
                      componentSurface: slots?.messageItem,
                    });
                    const colS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}-contentColumn`,
                      implementationBase: { flex: "1", style: { minWidth: 0 } },
                      componentSurface: slots?.contentColumn,
                    });
                    const hdS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}-header`,
                      implementationBase: {
                        display: "flex",
                        alignItems: "baseline",
                        gap: "sm",
                    ...
                    const auS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}-author`,
                      implementationBase: {
                        fontSize: "sm",
                        fontWeight: "semibold",
                        color: "var(--sn-color-foreground, #111827)",
                      },
                      componentSurface: slots?.authorName,
                    });
                    const tsS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}-timestamp`,
                      implementationBase: {
                        fontSize: "xs",
                        color: "var(--sn-color-muted-foreground, #6b7280)",
                      },
                    ...
                    const bdS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}-body`,
                      implementationBase: {
                        fontSize: "sm",
                        color: "var(--sn-color-foreground, #111827)",
                        lineHeight: "normal",
                    ...
                    const emS = resolveSurfacePresentation({
                      surfaceId: `${rootId}-message-${msgId}-embeds`,
                      implementationBase: {
                        display: "flex",
                        flexDirection: "column",
                        gap: "sm",
                    ...
                    return (
                      <div key={msgId}>
                        <div
                          ...
                          data-snapshot-id={`${rootId}-message-${msgId}`}
                          onClick={
                            onMessageClick
                              ? () => onMessageClick(msg)
                              : undefined
                        ...
                        >
                          ...
                          <div
                            data-snapshot-id={`${rootId}-message-${msgId}-contentColumn`}
                            className={colS.className}
                            style={colS.style}
                          >
                            {!isGrouped ? (
                              <div
                                data-snapshot-id={`${rootId}-message-${msgId}-header`}
                                className={hdS.className}
                                style={hdS.style}
                              >
                                <span
                                  data-snapshot-id={`${rootId}-message-${msgId}-author`}
                                  className={auS.className}
                                  style={auS.style}
                                >
                                  {authorName}
                                </span>
                                {showTimestamps && ts ? (
                                  <span
                                    data-snapshot-id={`${rootId}-message-${msgId}-timestamp`}
                                    className={tsS.className}
                                    style={tsS.style}
                            ...
                            <div
                              data-snapshot-id={`${rootId}-message-${msgId}-body`}
                              className={bdS.className}
                              style={bdS.style}
                              dangerouslySetInnerHTML={{ __html: sanitized }}
                            />
                            {showEmbeds && embeds.length > 0 && renderEmbed ? (
                              <div
                                data-snapshot-id={`${rootId}-message-${msgId}-embeds`}
                                className={emS.className}
                                style={emS.style}
                              >
                                {embeds.map((embed, ei) =>
                                  renderEmbed(embed, ei),
                                )}
                              </div>
                            ) : null}
                          </div>
                        </div>
                        <SurfaceStyles css={msgS.scopedCss} />
                        <SurfaceStyles css={colS.scopedCss} />
                        <SurfaceStyles css={hdS.scopedCss} />
                        <SurfaceStyles css={auS.scopedCss} />
                        <SurfaceStyles css={tsS.scopedCss} />
                        <SurfaceStyles css={bdS.scopedCss} />
                        <SurfaceStyles css={emS.scopedCss} />
                      </div>
                    );
                  })}
                </div>
