...
import { generateRootLayoutTopNav, generateTopNav } from "./templates/layouts/top-nav";
...
`;
  ...
  if (config.components.length > 0) {
    ...
    try {
      ...
      log.warn("shadcn init failed. You can run it manually: bunx shadcn@latest init");
    }

    s.start("Adding shadcn components");
    try {
      exec(`bunx shadcn@latest add ${config.components.join(" ")} --yes`, config.dir);
