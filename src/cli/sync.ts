...
export interface BackendConfig {
  ...
  zod?: boolean;
  /**
   * When set, `snapshot sync` generates server-side prefetch functions alongside
   * client hooks. Each GET endpoint gets a `prefetch<OperationName>.ts` file in
   * this directory. Uses React's `cache()` for deduplication within a single
   * server render.
   *
   * @example `"src/server/prefetch"`
   */
  prefetchDir?: string;
}
...
async function runSyncOnce(
  ...
  // ── Generate prefetch files (when prefetchDir is configured) ─────────────
  if (backend.prefetchDir) {
    await generatePrefetchFiles(
      backend.prefetchDir,
      cwd,
      byTag,
      componentSchemas,
...
/**
 * Generate server-side prefetch functions for GET endpoints.
 *
 * For each GET operation, generates a `prefetch<OperationName>.ts` file in
 * `prefetchDir`. Uses React's `cache()` for deduplication within a single
 * server render.
 *
 * @internal — called by `runSyncOnce` when `backend.prefetchDir` is set.
 */
async function generatePrefetchFiles(
  prefetchDirRelative: string,
  cwd: string,
  byTag: Map<string, TaggedOp[]>,
  componentSchemas: Record<string, SchemaObject>,
  apiDir: string,
  typesFile: string,
  logger: SyncLogger,
): Promise<void> {
  const prefetchDir = path.resolve(cwd, prefetchDirRelative);
  await fs.mkdir(prefetchDir, { recursive: true });

  const typesRelForPrefetch = toRelativeImport(prefetchDir, typesFile);
  const apiRelForPrefetch = toRelativeImport(prefetchDir, apiDir);
  ...
  for (const [tag, ops] of byTag.entries()) {
    for (const { method, pathStr, operation, pathLevelParams } of ops) {
      ...
      const fnName = plainFnName(method, pathStr, operation.operationId);
      const prefetchName = `prefetch${fnName.charAt(0).toUpperCase()}${fnName.slice(1)}`;

      const successSch = successSchema(operation.responses);
      const respType = successSch ? schemaToTs(successSch) : "void";

      const tagSlug = tag
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");
      const apiImport = `${apiRelForPrefetch}/${tagSlug}`;

      // Collect type refs for this operation
      const refs = new Set<string>();
      if (successSch) collectRefs(successSch).forEach((r) => refs.add(r));

      const typeImports =
        refs.size > 0
          ? `import type { ${[...refs].join(", ")} } from '${typesRelForPrefetch}';\n`
          : "";
      ...
      const content = [
        `// AUTO-GENERATED by snapshot sync — do not edit`,
        `// Regenerate: bun run sync`,
        `import { cache } from 'react';`,
        `import { ${fnName} } from '${apiImport}';`,
        ...(typeImports ? [typeImports.trimEnd()] : []),
        ``,
        `/**`,
        ...(summary ? [summary.trimEnd()] : []),
        ` * Server-side prefetch for ${operation.summary ?? pathStr}.`,
        ` *`,
        ` * Uses React's \`cache()\` — multiple calls with the same arguments within a`,
        ` * single server render share one network round trip (deduplication).`,
        ` *`,
        ` * Query key matches \`${hookName(method, fnName)}\`: \`${queryKeyComment}\``,
        ` *`,
        ` * @example`,
        ` * \`\`\`ts`,
        ` * import { ${prefetchName} } from '../../server/prefetch/${prefetchName}'`,
        ` *`,
        ` * export async function load(ctx: SsrLoadContext) {`,
        ` *   const data = await ${prefetchName}(${pathParams.map((p) => `ctx.params.${p.name}`).join(", ")})`,
        ` *   if (!data) return { notFound: true }`,
        ` *   return {`,
        ` *     data: { data },`,
        ` *     queryCache: [{ queryKey: [${pathParams.map((p) => `ctx.params.${p.name}`).join(", ")}], data }],`,
        ` *   }`,
        ` * }`,
        ` * \`\`\``,
        ` */`,
        `${deprecated}export const ${prefetchName} = cache(async (${fnArgs}): Promise<${respType}> => {`,
        `  return ${fnName}(${callArgs});`,
        `});`,
        ``,
      ].join("\n");

      const fileName = `${prefetchName}.ts`;
      const filePath = path.join(prefetchDir, fileName);
      await fs.writeFile(filePath, content, "utf8");

      const rel = path.relative(cwd, filePath).replace(/\\/g, "/");
      logger.success(`${rel} — prefetch (${operation.summary ?? pathStr})`);
    }
  }
}
...
export async function processManifest(
  ...
  if (manifest.routes.length > 0) {
    ...
    for (const route of manifest.routes) {
      const routeContent = generateRouteFile(
        route.path,
        route.page as PageConfig,
      );
      const fileName =
        route.path === "/"
          ? "index.tsx"
