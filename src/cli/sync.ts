...
export interface BackendConfig {
  ...
  zod?: boolean;
  /** Glob patterns for paths to include (e.g. ["/auth/**", "/users/**"]). If set, only matching paths are synced. */
  include?: string[];
  /** Glob patterns for paths to exclude (e.g. ["/admin/**", "/internal/**"]). Applied after include. */
  exclude?: string[];
}
...
/**
 * Converts an endpoint glob pattern to a RegExp.
 *   - `**` matches any number of path segments (including zero)
 *   - `*` matches a single path segment
 *   - Literal segments match exactly
 *   - OpenAPI path params like `{id}` match any single segment
 */
export function pathPatternToRegex(pattern: string): RegExp {
  ...
  for (const part of parts) {
    if (part === "**") {
      ...
      // leading slash produces empty first element — skip
      continue;
    } else {
...
/**
 * Filters OpenAPI paths by include/exclude glob patterns.
 *   - If include is set, only paths matching at least one include pattern are kept.
 *   - If exclude is set, paths matching any exclude pattern are removed (applied after include).
 *   - If neither is set, all paths pass through.
 */
export function filterPaths(
  ...
  for (const [pathStr, pathItem] of Object.entries(paths)) {
    if (hasInclude && !matchesAnyPattern(pathStr, include)) continue;
    if (hasExclude && matchesAnyPattern(pathStr, exclude)) continue;
    filtered[pathStr] = pathItem;
  }
  return filtered;
}
...
export interface SyncOptions {
  ...
  typesPath?: string;
  snapshotImport?: string;
  /** Glob patterns for paths to include. Passed to backends that don't define their own. */
  include?: string[];
  /** Glob patterns for paths to exclude. Passed to backends that don't define their own. */
  exclude?: string[];
...
async function runSyncOnce(
  ...
  // ── Filter paths by include/exclude ────────────────────────────────────────
  ...
  for (const [pathStr, pathItem] of Object.entries(filteredPaths)) {
...
// ─── Testing exports ─────────────────────────────────────────────────────────
// These are internal implementation details exported solely for unit testing.
// They are NOT part of the public API. Import from '../sync' in test files only.
