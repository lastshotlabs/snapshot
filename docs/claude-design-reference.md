...
<RowBase gap="md" justify="between" align="center">
  ...
</RowBase>;
```
...
interface NotificationFeedBaseProps {
  ...
  timestampField?: string;
  readField?: string;
  typeField?: string; // type: 'info'|'success'|'warning'|'error'
...
// Headless:
const { values, errors, handleChange, handleSubmit, isSubmitting, reset } =
  useAutoForm({
    fields: [
      { key: "title", label: "Title", inputType: "text", required: true },
    ],
...
interface DatePickerDisabledEntry {
  dayOfWeek: number[];
}
...
interface CommandPaletteBaseGroup {
  label: string;
...
```tsx
interface DropdownMenuBaseTrigger {
  label?: string;
  icon?: string;
}
...
<ChatWindowBase
  ...
  inputSlot={
    <RichInputBase sendOnEnter onSend={({ text }) => sendMessage(text)} />
  }
/>;
```
...
class JeopardySocket {
  ...
  submitAnswer(answer: string) {
    this.send({
      type: "game:input",
      sessionId: this.matchId,
      channel: "regularAnswer",
      input: { answer },
    });
  }
  submitFinalWager(amount: number) {
    this.send({
      type: "game:input",
      sessionId: this.matchId,
      channel: "finalWager",
