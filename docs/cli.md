...
with your slingshot backend's OpenAPI schema.
...
Scaffolds a complete React application wired to a slingshot backend.
...
Use this for local development against a slingshot backend before auth is configured.
...
│
├── server-plugins/
│   └── mail.ts                     # slingshot-mail config (only if mail plugin selected)
│
└── .gitignore
```
...
# 1. Fill in .env
#    VITE_API_URL=http://localhost:3001   ← your slingshot backend
...
Given an OpenAPI schema, sync writes to the paths configured in the
`snapshot` field of `package.json`:
...
your slingshot backend writes `schema.json` on restart.
...
The `snapshot` field in `package.json` controls where `snapshot sync` writes its
output. `snapshot init` generates it with the app scaffold.
...
`createSnapshot()` is called once in this file. The exported hooks are closures
over that single runtime instance.
...
changes). If using `--watch`, the file must change on disk — slingshot writes
...
looping, check that your slingshot backend's session endpoint (`/auth/me` or equivalent)
