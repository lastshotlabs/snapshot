...
### 2. Snapshot `--sn-*` token system

Snapshot UI components use a separate `--sn-*` token system documented in
[tokens.md](./tokens.md). These tokens are generated by `resolveTokens()` and
injected as a `<style>` block.

**The two systems are independent.** Snapshot UI components read `--sn-color-primary`,
...
Snapshot UI components own their own visual appearance via `--sn-*` tokens. You
generally don't need to reference them in your custom code. But if you're building a
custom component that should visually match Snapshot UI, you can reference the
tokens directly:
...
This is only needed if you're building custom components that sit alongside Snapshot UI
and must visually match it. For most custom code, the scaffold theme variables
...
have independent default values. If your app uses both Snapshot UI and custom React
...
This is only needed for tight visual consistency between custom and Snapshot UI
