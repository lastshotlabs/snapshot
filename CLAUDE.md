...
**Shadows:** `--sn-shadow-none`, `--sn-shadow-xs`, `--sn-shadow-sm`, `--sn-shadow-md`,
`--sn-shadow-lg`, `--sn-shadow-xl`

**Z-index:** `--sn-z-index-base`, `--sn-z-index-dropdown`, `--sn-z-index-sticky`,
`--sn-z-index-overlay`, `--sn-z-index-modal`, `--sn-z-index-popover`, `--sn-z-index-toast`

**Animation/Transitions:** `--sn-duration-instant`, `--sn-duration-fast`,
`--sn-duration-normal`, `--sn-duration-slow`,
`--sn-ease-default`, `--sn-ease-in`, `--sn-ease-out`, `--sn-ease-in-out`, `--sn-ease-spring`

**Opacity:** `--sn-opacity-disabled`, `--sn-opacity-hover`, `--sn-opacity-muted`

**Line-height:** `--sn-leading-none`, `--sn-leading-tight`, `--sn-leading-normal`,
`--sn-leading-relaxed`, `--sn-leading-loose`

**Letter-spacing:** `--sn-tracking-tight`, `--sn-tracking-normal`, `--sn-tracking-wide`
...
21. **Design tokens are the styling boundary** — Components consume tokens, not raw CSS values.
    ...
    token switch, not a separate theme. Breakpoint-aware tokens accept a flat value or a
    breakpoint map: `{ "default": 1, "md": 2, "lg": 3 }`.
