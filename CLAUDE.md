...
Snapshot is the frontend SDK + CLI for bunshot-powered backends:
...
activate, it is incomplete. The manifest bootstrap layer must wire everything — components,
data sources, actions, from-ref bindings, guards, auth, navigation — without caller-supplied
function references. SSR integration (route resolution, preloads, guards, head/meta) must
also be manifest-declarable.

Test: "Can a user enable this by editing manifest JSON with no TypeScript?" If no, the
design is wrong. If you implement something that doesn't work in manifest mode, flag it
immediately.
...
Follow `docs/spec-process.md`. Audit codebase first, surface decisions, resolve ambiguity,
separate tracks on separate branches.
